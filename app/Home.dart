...
Future<List<dynamic>> fetchFacultyCurrentData(String email) async {
  final response = await http.get(Uri.parse('http://device-ip:3001/fetchfacultycurrentdata?email=$email'));
...
Future<List<dynamic>> fetchFacultyFutureData(String email) async {
  final response = await http.get(Uri.parse('http://device-ip:3001/fetchfacultyfuturedata?email=$email'));
  if (response.statusCode == 200) {
    return jsonDecode(response.body)['data'];
  } else {
    throw Exception('Failed to fetch data');
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  bool _currentdataisfetched=false;
  bool _inRange=false;
  ...
  Future<void> _fetchCurrentData() async {
    try {
      final data = await fetchFacultyCurrentData(widget.email);
      if (data.isNotEmpty) {
        setState(() {
          _data = data;
          _currentdataisfetched = true;
        });
      }
    } catch (e) {
      print(e);
  ...
  Future<void> _fetchFutureData() async {
    try {
      final data = await fetchFacultyFutureData(widget.email);
      if (data.isNotEmpty) {
        setState(() {
          _futuredata = data;
          _dataisfetched = true;
  ...
  Future<bool> isWithinLocation() async {
    double lat = 1.00000;//replace with latitute constraint value
    double long = 2.00000;//replace with longitude constraint value
  ...
  void navigateToMarkAttendance() {
    if (_inLocation && _isSessionTime &&_markbuttonvisible) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Attendance not allowed outside of session timings or outside of defined location or your attendance is already marked.',style: TextStyle(color: Colors.red)),
  ...
  void navigateToPaperCollection() {
    if (_inLocation && _isSessionTime &&_papercollectionbuttonvisible) {
      ...
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Paper collection not allowed outside of session timings or outside of defined location or paper collection is already marked.',style: TextStyle(color: Colors.red)),
        duration: Duration(seconds: 4),
      ));
    }
  }
          ...
          children: [
            ...
            Center(
              child: Visibility(
                ...
                child: Card(
                  margin: const EdgeInsets.fromLTRB(50.0, 5.0, 50.0, 10.0),
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: <Widget>[
                      ListTile(
